...
# use prophet for everything (not minutes driven)
bball_allstats_forecast <- function(df_list, contest_date) {
  library(prophet)
  pt_projections <- as.data.frame(matrix(ncol = 5, nrow = length(df_list)))
  colnames(pt_projections) <- c('player_id', 'player_name', 'yhat', 'yhat_lower', 'yhat_upper')
  
  assists <- c('ended_at', 'assists')  
  for (i in 1:length(df_list)) {
    temp <- as.data.frame(df_list[i])
    assists <- temp[, grepl(paste(assists, collapse = '|'), names(temp))]
    colnames(assists)[1] <- 'y'
    colnames(assists)[2] <- 'ds'
    assists$y[is.na(assists$y)] <- 0
    assists <- prophet(assists)
    future <- make_future_dataframe(assists, periods = 14)
    forecast <- predict(assists, future)
...
# generate forecasted scoring range with scoring rules in here? 
